...
fn seventeenth_test() {
    ...
    let choice = eval::top_level_search(&mut b, 4, 69420).unwrap();
    println!("{:?}", choice);
}
...
fn fourteenth_test() {
    let fen_string = "r1bqkbnr/pppp1ppp/2n5/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR b KQkq - 3 3";
    let mut b = utils::Board::from_fen(fen_string);
    let choice = eval::top_level_search(&mut b, 6, 69420).unwrap();
    // 6 - 2 s
    // 7 - 10 s
    // 8 - 1 min
    println!("{:?}", choice);
...
fn thirteenth_test() {
    let mut b = utils::Board::from_fen("r1bqkb1r/pppp1ppp/2n4n/4p2Q/2B1P3/8/PPPP1PPP/RNB1K1NR w KQkq - 4 4");
